...
class Product extends AppModel
{
    ...
    {
        $filters = \R::getCol("SELECT attr_id FROM attribute_product WHERE product_id = ?", [$id]);

        //если менеджер убирает фильтры - удаляем их
        if (empty($data['attributes']) && !empty($filters)) {
            \R::exec("DELETE FROM attribute_product WHERE product_id = ?", [$id]);
            return;
        }

        // если фильтры добавляются
        if (empty($filters) && !empty($data['attributes'])) {
            $sql_part = '';
        ...
        //если изменились фильтры - удаляем старые, записываем новые.
        if (!empty($data['attributes'])) {
            $result = array_diff($filters, $data['attributes']);
            if ($result) {
                \R::exec("DELETE FROM attribute_product WHERE product_id = ?", [$id]);
                $sql_part = '';
                foreach ($data['attributes'] as $value) {
    ...
    public function editRelatedProduct($id, $data)
    {
        ...
        //если менеджер убирает связанные товары - удаляем их
        ...
        //если изменились связанные товары - удаляем старые, записываем новые.
